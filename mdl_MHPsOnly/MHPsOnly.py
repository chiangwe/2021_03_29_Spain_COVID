...
for each_date in tqdm(date_ranges):
	...
	for itr_em in range(0, max_itr):
		...
		R0_est = tf.math.reduce_sum(P_j[:, :, 0:n_dates-ds_crt], axis=2)/(n_dates-ds_crt)
		R0_est = R0_est.numpy()
		...
		if bool_wbl:
			...
			if alpha_shape_est < 1.2:
				alpha_shape_est = 1.2;
			if beta_scale_est < 1.2:
				beta_scale_est = 1.2;
		...
		# ----- Early Stop -------#
		if itr_em > 3:
			if (	  np.all(np.array( dict_delta['delta_R'][-3:])  < tol) \
