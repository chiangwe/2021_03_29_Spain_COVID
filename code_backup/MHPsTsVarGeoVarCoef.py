...
# Change Log, Lat into Radius
df_demo['lon'] = [radians(_) for _ in df_demo['lon']]
df_demo['lat'] = [radians(_) for _ in df_demo['lat']]
tol_radi = np.array( [df_demo['lat'].values, df_demo['lon'].values] ).T
...
#----- Map Logtitude and Latitude -----#
arr_dist = np.array( [ (lat, lon) for lat, lon in  zip(df_demo['lat'], df_demo['lon'])] );
arr_dist = dist.pairwise(arr_dist);

#dict_indGeo = dict(zip( df_demo.index,  [ (lat, lon) for lat, lon in  zip(df_demo['lat'], df_demo['lon'])] ))


#auto_bwz = dist.pairwise(tol_radi).mean()


# Calcuate auto-bandwidth
...
parser.add_argument("--max_itr", type=int, help="# of maximum iterations", default=120)
parser.add_argument("--mode", type=str, help="# of maximum iterations", default='server')
parser.add_argument("--n_proc", type=str, help="Number of processes", default=5)
#
parser.add_argument("--mdl_name", type=str, help="# of maximum iterations", default='MHPsTsVarGeoVarCoef')
parser.add_argument("--case_type", type=str, help="# of maximum iterations", default='confirm')
parser.add_argument("--bw", type=str, help="bandwidth for the kernel", default=10)
...
print("bw: ", bw, " alpha_shape: ", alpha_shape, " beta_scale: ", beta_scale)

# define coefficient

def PR_fit( q_PR, q_job):
    ...
    while(not q_job.empty()):
        ...
        #new_weight_zone = dist.pairwise(tol_radi[COV_X_infZ,:], tol_radi[each_zone:each_zone+1, :]).flatten()/bwz
        #new_weight_zone = (COV_X_infZ - each_zone)/bwz;
        #new_weight_zone = np.exp( -(new_weight_zone ** 2)/2 )/(np.sqrt(2*np.pi))/bwz
        #new_weight = np.multiply( new_weight, new_weight_zone)
...
for each_date in date_ranges:
    ...
    while(time_out== True):
        ...
        # ----- Set initial parameters -----#
        para_str = 'type_' + case_type + '_bw_' + str(bw)  + '_alpha_' + str(alpha_shape) + '_beta_' + str(beta_scale) + '_predat_';
        ...
        # Append pairwise distance
        arr_dist_append = np.expand_dims( arr_dist, axis=2 );
        arr_dist_append = np.expand_dims( arr_dist_append, axis=3 );
        arr_dist_append = np.tile( arr_dist_append,  [1, 1, n_dates, 1] )
        COV = np.concatenate( (COV, arr_dist_append ), axis=3)
        COV_name = COV_name + ['distance']
        n_feat = len(COV_name)
        ...
        #
        COV_infZ    = np.unravel_index( range(0, COV.shape[0]), (n_hzone, n_hzone, n_dates ), order='F')[0];
        COV_X_infZ  = np.unravel_index( range(0, COV_X.shape[0]), (n_hzone, n_hzone, n_dates_tr - ds_crt), order='F')[0];
        COV_te_infZ = np.unravel_index( range(n_dates_tr, n_dates_tr+n_dates_te), (n_hzone, n_hzone, n_dates), order='F')[0];
        ...
        zone_processes = range(0, n_hzone);
        ...
        for itr_em in range(0, max_itr):
            ...
            for each_proc in range(0, n_proc):
                p = multiprocessing.Process(target=PR_fit, args=(q_PR, q_job))
                p.start()
                ls_p.append(p)

            #start = time.time()
            for itr_t in tqdm(s_processes):
                try:
                    each_t, coef, r0 = q_PR.get(timeout=60)  # Returns output or blocks until ready
