...
def off_sim( q_sim, q_job):
	...
	while(True):
		...
		offspring = np.reshape(offspring, [n_hzone, 1]).sum(1);
...
if __name__ == '__main__':
	...
	parser.add_argument("--alpha_shape", type=str, help="Shape parameters for wbl", default=0)
	parser.add_argument("--beta_scale", type=str, help="bandwidth for the kernel", default=0)
	...
	for each_date in tqdm(date_ranges, desc='processes on dates'):
		...
		if bool_wbl == True:
			...
			alpha_shape = int(alpha_shape)
			beta_scale = int(beta_scale)
		...
		if n_dates_tr + d_pred > sim_dates:
			d_pred = int(n_dates_te/7)*7
		# == Get R0 first ==#

		#R0_est = np.tile( np.expand_dims( np.eye( mdic['R0'].shape[0] ) * mdic['R0'], axis=2), [1, 1, n_dates] )
		#R0_est[R0_est>1] = 1
		...
		tf_R0 = tf.convert_to_tensor(R0_est,  name='tf_R0');  # print("tf_R0: ", type(tf_R0), tf_R0.shape)
		#
		...
		#R0_est_sim = R0_est
		R0_est = np.tile(np.expand_dims(R0_est, axis=2), [1, 1, n_sim])
		R0_est[R0_est>2] = 2

		new_mu = tf_lamb.numpy()[:, n_dates_tr: n_dates_tr + d_pred]
		new_mu = np.tile(np.expand_dims(new_mu, 2), [1, 1, n_sim])

		output_cnt = output_cnt + np.random.poisson(new_mu.flatten(order='F')).reshape((n_hzone, d_pred, n_sim), order='F')
		
		tim_indx = np.expand_dims(np.expand_dims(np.arange(0, d_pred), axis=0), axis=2)
		tim_indx = np.tile(tim_indx, [n_hzone, 1, n_sim])

		row_indx = np.expand_dims(np.expand_dims(np.arange(0, n_hzone), axis=1), axis=2)
		row_indx = np.tile(row_indx, [1, d_pred, n_sim])

		sim_indx = np.expand_dims(np.expand_dims(range(0, n_sim), axis=0), axis=0)
		sim_indx = np.tile(sim_indx, [n_hzone, d_pred, 1])

		# print(tim_indx.shape, row_indx.shape, R0_est_sim.shape, output_cnt.shape )

		for itr_pred in tqdm(range(0, d_pred)):
			
			output_cnt_in = output_cnt.copy();

			# Put each t in que
			for itr_sim in range(0, n_sim):
				r_0s = R0_est[:, 0, itr_sim].flatten(order='F')
